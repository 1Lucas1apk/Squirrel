...
export function DevendoScreen({ dividas, onCriar, onExcluir, onToggle, isFechado }: DevendoScreenProps) {
  ...
  const handleExcluir = (id: string) => {
    Alert.alert(
      "Remover Troco",
      "Deseja realmente apagar esta dívida de troco? Esta ação não pode ser desfeita.",
      [
      ...
      <View className="gap-4">
        ...
        {dividas.map((item) => (
          <View key={item.id} className={`rounded-[32px] border p-6 ${item.resolvido ? "border-zinc-800 bg-zinc-900/50 opacity-40" : "border-orange-500/20 bg-ink-900"}`}>
            ...
            <Pressable disabled={isFechado} onPress={() => onToggle(item)} hitSlop={15} style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 12, borderRadius: 20, borderWidth: 2, paddingVertical: 18, backgroundColor: item.resolvido ? '#10b981' : 'rgba(251, 146, 60, 0.1)', borderColor: item.resolvido ? '#10b981' : 'rgba(251, 146, 60, 0.3)', opacity: isFechado ? 0.8 : 1 }}>
